...
static const char *EXPECTED_OWNER = "salim_ghee";
...
int main(int argc, char **argv) {
    const char *invoker = (argc > 1) ? argv[1] : "/home/linuxIPN3/decrypt.sh";
    uid_t expect = expected_uid();
    struct stat st;

    /* Validate the wrapper script ownership. */
    if (expect == (uid_t)-1 || stat(invoker, &st) != 0 || st.st_uid != expect) {
        const char *msg = "Access denied: decrypt script must be owned by \"salim_ghee\"\n";
